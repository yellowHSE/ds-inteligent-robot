...
- RGB는 compressed(JPEG)로 구독 → 네트워크 대역폭 절감
- Depth는 raw(16UC1/32FC1)로 구독 → CPU 디코딩 오버헤드 방지
...
class YoloDepthNode(Node):
    def __init__(self):
        ...
        # 토픽/모델/추론
        # RGB는 compressed로 기본값 설정
        self.declare_parameter('rgb_topic', '/robot3/oakd/rgb/image_raw/compressed')
        self.declare_parameter('depth_topic', '/robot3/oakd/stereo/image_raw')  # raw 유지 권장
        ...
        self.declare_parameter('publish_annotated', False)  # 네트워크 절약 기본값
        ...
        # ===== 구독 + 동기화 =====
        # RGB는 CompressedImage, Depth는 raw Image
        ...
        self.get_logger().info(
            f"Subscribed: RGB(compressed): {self.rgb_topic}  DEPTH(raw): {self.depth_topic}  CameraInfo: {self.camera_info_topic}"
        )
    ...
    def _depth_to_vis(self, depth_np: np.ndarray, normalize_range_m: float) -> np.ndarray:
        ...
        vis = (depth_m / normalize_range_m * 255.0).astype(np.uint8)  # typo fix below
        return vis
